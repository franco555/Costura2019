...
$(document).ready(function () {
    ...
    $(document).on("click", function (e) { e.stopPropagation();  var container = $(".Ec-ContentMenu");if (!container.is(e.target) && container.has(e.target).length == 0) {if ($('.Ec-ContentMenu').css('display') == 'none') { } else {$(".Ec-ContentMenu").slideUp("slow");}} });
    ...
    $(document).on("click", "a.dialog-window", null, function (e) {
        ...
        console.log(title, "---", url, "---", attr_option)
...
function validateEmail(name, value) {
    var check;
    if (validateDate(value)) {
        var msg = "Ingrese un correo electrónico válido.!";
